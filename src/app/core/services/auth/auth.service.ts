...
import {
  ...
  UsuariosWebClosterInterface,
} from 'src/app/models/usuarios-web-closter-interface';
@Injectable({
  providedIn: 'root',
})
export class AuthService {
    ...
    return this.http.post<LoginResponseInterface>(url.toString(), {
        ...
        contrasena: contrasena,
          ...
          if (res.token) {
            ...
            // console.log('Verifica si existe la propiedad "nombreUsuario" en el objeto de arriba');
            // console.groupEnd();
   ...
   * @returns El objeto de usuario autenticado o null si no existe.
   */
  ...
  rolUsuario = new BehaviorSubject<UsuariosWebClosterInterface | null>(
    this.getCurrentData(),
  );
  rolUsuarioLogeado$ = this.rolUsuario.asObservable();

  datosUsuario = new BehaviorSubject<UsuariosWebClosterInterface | null>(
    this.getCurrentData(),
  );
  ...
  actualizarRol() {
    ...
    this.datosUsuario.next(this.getCurrentData());
    // console.log("datosUsuario", this.datosUsuario.value)
